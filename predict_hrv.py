# predict_hrv.py - HRV-based AFib Prediction
import argparse
import numpy as np
import pandas as pd
import io
import contextlib
from r_peak_detector import detect_r_peaks
from hrv_feature_extractor import extract_hrv_features
from model_loader_and_predictor import load_model_and_predict

def predict_from_file(file_path: str, fs: int = 250, out: str = None):
    """
    Stand-alone prediction function. Returns a result dictionary.
    """
    logs = io.StringIO()
    with contextlib.redirect_stdout(logs):
        # 1. Load signal
        if file_path.endswith('.csv'):
            df = pd.read_csv(file_path)
            signal = df.iloc[:, 0].values  # First column
        elif file_path.endswith('.npy'):
            signal = np.load(file_path)
        elif file_path.endswith('.txt'):
            signal = np.loadtxt(file_path)
        elif file_path.endswith('.mat'):
            import scipy.io
            mat = scipy.io.loadmat(file_path)
            # Find the signal key (usually 'val' or something similar)
            signal_key = [k for k in mat.keys() if not k.startswith('__')][0]
            signal = mat[signal_key].flatten()
        else:
            raise ValueError("Supported: .csv, .npy, .txt, .mat")

        print(f"Signal loaded: {len(signal)} samples, {len(signal)/fs:.1f} seconds")

        # 2. Detect R-peaks
        r_peaks = detect_r_peaks(signal, fs=fs)
        print(f"R-peaks detected: {len(r_peaks)}")

        if len(r_peaks) < 3:
            raise ValueError("Too few R-peaks detected. Please check signal quality.")

        # 3. Extract features
        features = extract_hrv_features(r_peaks, fs=fs)
        feature_vec = np.array(list(features.values())).reshape(1, -1)

        # 4. Predict
        result = load_model_and_predict(feature_vec)
        result["num_beats"] = len(r_peaks) - 1
        result["signal_duration"] = len(signal)/fs
        
        # 5. Save to JSON if needed
        if out:
            import json
            with open(out, 'w') as f:
                json.dump(result, f, indent=2)

    result["raw_logs"] = logs.getvalue()
    return result

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Predict AFib from ECG file")
    parser.add_argument("file_path", help="Path to ECG file")
    parser.add_argument("--fs", type=int, default=250, help="Sampling rate (default: 250)")
    parser.add_argument("--out", help="Output JSON file")
    args = parser.parse_args()

    res = predict_from_file(args.file_path, fs=args.fs, out=args.out)
    print(res["raw_logs"])
    print(f"Label: {res['label']} ({res['probability_afib']*100:.1f}%)")